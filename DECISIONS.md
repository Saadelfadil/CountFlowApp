...
## D-078 — Percentage text is a per-binding toggle, never a per-style omission; Minimal/Glass/Rounded's WIDE identity anchors to the left edge, not the center of its stretched column

**Date:** 2026-08-12 · **Status:** Accepted · **Milestone:** 5A follow-up (Session 19)

Three real Samsung Galaxy A55 findings in the real Glance widget renderer (`CountdownWidgetLayouts.kt`), none of them in the entitlement/AdMob/persistence code any adjacent session touched.

**Percentage omission.** `MinimalLayout`, `MinimalLayoutWide`, `GlassLayout`, `GlassLayoutWide`, `RoundedLayout`, and `RoundedLayoutWide` never read `model.showPercentageText` at all — the toggle was silently a no-op for exactly these three styles, while `MaterialLayout`, `OledLayout`, and `ModernLayout` (and their `Wide` forms) already read it correctly. `WidgetPreviewCard.kt` was never wrong: it has drawn percentage unconditionally on style since it was written, which is exactly why the Customize Widget preview and the real placed widget disagreed. An existing test, `minimal never draws percentage text, even when progress is visible and requested`, had encoded the omission as intended behavior; corrected to assert the fix instead, since physical-device evidence and this task's own explicit truth table (percentage visibility depends only on the binding's own toggle, never on progress style or widget style) supersede that earlier, undocumented assumption. `docs/WIDGET_DESIGN_GUIDE.md`'s Minimal entry made the same assumption explicit ("No progress bar... the absence of a progress bar, full stop") — also stale since Style and Progress became independent settings, corrected in the same pass; `docs/WIDGET_SIZE_MATRIX.md`'s wider staleness on this point (Minimal and OLED's Progress column both still say "Never") is flagged at the top of that document rather than corrected cell-by-cell, since a full pass over all 21 combinations is a larger documentation task than this fix's own scope.

**STANDARD vertical budget.** Adding a percentage line to three single-column STANDARD layouts that previously never drew one reopened the same real ~200dp-budget risk `SECONDARY_RING_DP_OLED_STANDARD` (44dp, vs the generic 56dp) already exists to manage for OLED. Rather than inventing new numbers, `SECONDARY_RING_DP_STANDARD_COMPACT` reuses OLED's own already-reasoned 44dp value for Minimal/Glass/Rounded's STANDARD ring, paired with a tightened pre-ring spacer (`SPACING_XS` in place of `SPACING_SM`/`SPACING_LG`) and the percentage text placed directly after the graphic with no additional spacer — matching `MaterialLayout`'s own existing, unremarked pattern of putting percentage immediately after its progress graphic. `MinimalLayoutWide`'s single-column WIDE form got the analogous `SECONDARY_RING_DP_WIDE_SINGLE_COLUMN_COMPACT` (34dp, down from 40dp) for the same reason, reasoned from OLED's WIDE value (36dp) rather than newly measured.

**WIDE dead zone.** `GlassLayoutWide` and `RoundedLayoutWide`'s left ("identity") column used `CenteredIdentity` (or an explicit `horizontalAlignment = CenterHorizontally`) inside a `defaultWeight()`-stretched column — centering identity across the *entire* leftover width of a wide card, not just its own natural content width, so identity visually drifted toward the middle of the card while the headline/ring column sat flush right, leaving the large accidental empty region the brief's own Glass example named. `MaterialLayoutWide` and `ModernLayoutWide` never had this bug — both already used `StartIdentity`, anchoring identity to the left edge of the stretched column regardless of its width. Fixed by switching Glass/Rounded's WIDE identity to the same `StartIdentity` pattern (and, for Glass, dropping `TextAlign.Center` from its secondary line, which had the same stretched-centering problem).

**Reason.** All three findings share one root cause: a composable's own explicit `Center`-flavored alignment interacting badly with a `defaultWeight()`/stretched container it wasn't originally written against — either "this style just never got a percentage slot added" (an omission that looks intentional once a test encodes it) or "centering inside a small box" (correct) silently becoming "centering inside a much bigger box" once that box started stretching to fill leftover WIDE width (a bug). Preview parity was the forcing function for the first: `WidgetPreviewCard.kt` never had per-style exceptions to begin with, so it was always right, and the real widget was always what needed to catch up.

**Alternatives.** For the STANDARD budget: shrinking every layout's headline font globally — rejected explicitly by this task's own brief, and unnecessary once the actual crowded region (the ring/percentage block) had its own margin to give back. A per-toggle dynamic ring size (larger when percentage is off, smaller when on) — rejected: the brief's own "keep it visually consistent" instruction rules out a ring that changes size depending on unrelated toggle state, and a fixed worst-case size is simpler and only costs a few dp when percentage happens to be off. For the WIDE dead zone: giving the left column a fixed/intrinsic width instead of `defaultWeight()` — rejected in favor of matching `StartIdentity`'s already-proven, minimal-diff pattern rather than restructuring the Row's own weighting, which no other WIDE layout needed to do.

**Tradeoffs.** None of the new spacing/ring-size values were re-measured on a physical device this session — they are reasoned from OLED's own already-real-device-informed constants, the same honesty standard `SECONDARY_RING_DP_OLED_STANDARD`'s own original comment already holds itself to. Robolectric's Glance testing API has no bounds/position assertion, so the WIDE dead-zone fix is verified only by "content still renders correctly post-fix," not by an assertion that the gap actually shrank — real confirmation is the physical-device QA pass this fix was written for.

---

## D-079 — Three widget-picker entries (Compact/Square/Wide), one shared renderer; each declares its own default footprint via `targetCellWidth`/`targetCellHeight`, never a permanent per-provider render mode

**Date:** 2026-08-13 · **Status:** Accepted · **Milestone:** 5A follow-up

Samsung's own Battery widget (and most system widgets) exposes multiple independently-selectable picker entries, each with its own default footprint/preview — a native Android/One UI concept CountFlow had no equivalent of before this: the single `CountdownGlanceWidgetReceiver` this app shipped with only ever offered one picker entry, defaulting to 2×2, with no way for a user to choose "start me at 2×1" or "start me at 4×2" from the picker itself (resizing after placement was, and remains, the only route to the other two footprints).

**Architecture.** Android ties an app-widget picker entry to a distinct `<receiver>` component — there is no supported way to register one `GlanceAppWidgetReceiver` under multiple picker entries with different `AppWidgetProviderInfo` metadata. Three thin `GlanceAppWidgetReceiver` subclasses now exist — `CountdownGlanceWidgetReceiver` ("CountFlow Square," 2×2 default; the original, unrenamed provider, kept stable specifically so widgets end users have already placed under it are not orphaned by this change — Android ties a placed widget instance permanently to its provider's exact component name), `CountdownGlanceWidgetReceiverCompact` ("CountFlow Compact," 2×1 default), and `CountdownGlanceWidgetReceiverWide` ("CountFlow Wide," 4×2 default) — sharing one new `BaseCountdownGlanceWidgetReceiver` (`@AndroidEntryPoint`, Hilt's documented pattern for a hierarchy of `@AndroidEntryPoint`-annotated classes) that holds every real behavior: `override val glanceAppWidget: GlanceAppWidget = CountdownGlanceWidget()` and the `onDeleted` cleanup, both unchanged. All three point at the exact same `CountdownGlanceWidget` class — the six Style renderers, `WidgetRenderMapper`, and the responsive Compact/Standard/Wide layouts exist exactly once, never duplicated per picker entry. Each receiver's own `res/xml/countdown_widget_info_*.xml` (compact/square/wide) differs only in `targetCellWidth`/`targetCellHeight` (the *default* placement footprint), `android:description`, and `android:previewLayout`; `minWidth`/`minHeight`/`maxResizeWidth`/`maxResizeHeight`/`resizeMode`/`widgetFeatures`/`configure` are identical across all three, verified by a new test (`all three providers allow resizing across the full 2x1 to 4x2 range, not just their own default`) — a widget placed through any picker entry remains freely resizable to either of the other two footprints from the home screen, exactly as the original single provider always was. `android:configure` on all three points at the same, unmodified `WidgetConfigurationActivity`.

**Picker previews.** `android:previewLayout` (API 31+, this app's own `minSdk`, so no `previewImage` fallback is needed) already existed for the original Square entry (`widget_preview.xml`, Session 9) — renamed `widget_preview_square.xml`, content unchanged. Two new sibling static Android layouts (`widget_preview_compact.xml`, `widget_preview_wide.xml`) follow the same "deliberately realistic static approximation, fixed sample content, never a live render" contract, but each mirrors its own real renderer's actual content hierarchy rather than a scaled copy of Square's: Compact omits the secondary line and progress bar entirely, matching COMPACT's own established "never draws a secondary line, a progress indicator, or a percentage" rule (`docs/WIDGET_SIZE_MATRIX.md`) — a picker preview that showed those anyway would advertise a widget that footprint can never actually render; Wide uses the same two-region horizontal composition (context left, countdown right, percentage beside the graphic) Sessions 20/21 established for the real WIDE renderer, so the Wide picker entry visually reads as genuinely different from Square rather than a taller/shorter crop.

**The `pruneOrphanedBindings` bug this architecture required fixing.** `GlanceWidgetRefreshScheduler.pruneOrphanedBindings()` queried `AppWidgetManager.getAppWidgetIds()` for exactly one `ComponentName` (`CountdownGlanceWidgetReceiver`) to determine which `WidgetBinding` rows are "live" versus orphaned. Adding two more receiver classes without updating this method would have made every widget placed through the Compact or Wide picker entry look orphaned to this method the moment it ran at the next app start, silently deleting their otherwise-valid bindings — found by tracing the existing background-refresh path while implementing this architecture, not reported independently. Fixed by unioning `getAppWidgetIds()` across all three receiver classes (`COUNTDOWN_WIDGET_PROVIDER_CLASSES`, `internal` for a dedicated unit test to assert against without needing a mocked `AppWidgetManager`).

**Save → Glance update path — verified, not modified.** `WidgetConfigurationActivity.onEventBound()` already did exactly what official Glance guidance describes: obtain the `GlanceId` via `GlanceAppWidgetManager(context).getGlanceIdBy(appWidgetId)`, call `CountdownGlanceWidget().update(context, id)` for that one instance, then `setResult(RESULT_OK, ...)` with `EXTRA_APPWIDGET_ID` and `finish()`. Verified directly against Glance 1.1.1's own sources (extracted from the library's sources jar, this project's established practice for library API claims): `GlanceAppWidgetManager.getGlanceIdBy(appWidgetId: Int)` only validates `AppWidgetManager.getAppWidgetInfo(appWidgetId)` is non-null and wraps `AppWidgetId(appWidgetId)` — provider-agnostic, no dependency on which of the three receivers placed the widget — and `GlanceAppWidget.update(context, id)` targets exactly the one `appWidgetId`, never `updateAll()`. No code change was needed or made here; this session's job was tracing and confirming it, not building it.

**Reason.** The three-picker-entry structure is the only Android-supported way to offer distinct default footprints from the widget picker (`AppWidgetProviderInfo` is per-`<receiver>`); sharing one `GlanceAppWidget` class across all three is what keeps that platform requirement from becoming three duplicated renderers. Keeping the original receiver's component name unchanged, rather than renaming all three symmetrically, is the safer choice given widgets are already placed on the owner's own Samsung Galaxy A55 test device under that exact name.

**Alternatives.** A single provider with `widgetFeatures="reconfigurable"` and one `targetCellWidth`/`targetCellHeight` pair — rejected: this is what the app already had, and it structurally cannot expose more than one default-footprint choice in the picker, which is the entire point of this task. Renaming all three receivers symmetrically (including the original) for naming consistency — rejected per Reason above; the asymmetry (one legacy name, two new ones) is a one-time, well-documented cost, not an ongoing maintenance burden, since `BaseCountdownGlanceWidgetReceiver` is where all three's real logic actually lives. A `previewImage` (static drawable/PNG) instead of `previewLayout` — rejected: this `minSdk` (31) already clears the API 31 requirement for `previewLayout`, which the app already used for its one prior picker entry, so no backward-compatibility fallback was needed.

**Tradeoffs.** Lint's `HardcodedText` check fires 4 and 6 times respectively on the two new static preview layouts (`widget_preview_compact.xml`, `widget_preview_wide.xml`), the same category of warning the original `widget_preview.xml`/`widget_preview_square.xml` has always carried for its own fixed sample content — not suppressed, following that file's own established precedent rather than adding new `tools:ignore` noise. Total lint warnings move from 18 to 29 as a result (0 errors, unchanged); a `DisableBaselineAlignment` suggestion on the new Wide layout's horizontal row *was* fixed (`android:baselineAligned="false"`), since that one was a genuine, free improvement rather than an accepted static-content tradeoff. No physical-device confirmation yet that the Samsung One UI widget picker actually renders three distinct "CountFlow Compact/Square/Wide" entries with the expected previews — the physical acceptance test this task specifies is this session's own explicitly deferred next step, not something Robolectric or a JVM unit test can confirm.
